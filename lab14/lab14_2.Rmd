...
```{r}
ui <- fluidPage(titlePanel("Log 10 Homerange Density"), selectInput("x", "Select X variable", choices = c("birds", "lake fishes", "mammals", "marine fishes", "river fishes", "snakes", "tortoises", "turtles"), selected = "mammals"),
...
server <- function(input, output, session) {
  output$plot <- renderPlot ({
    ...
    homerange %>%
      ...
      ggplot(aes(x = log10.hra)) + geom_density(fill = "steelblue", color = "black", alpha = .8)
