...
library("tidyverse")
install.packages("janitor_2.2.0.zip")
library("janitor")
```
...
```{r}
mutate(fish, length_half = length/2)
```
...
```{r}
arrange(mutate(select(mammals, family, genus, species, max_life), max_life_new = ifelse(max_life == -999.00, NA, max_life)), desc(max_life_new))
```
