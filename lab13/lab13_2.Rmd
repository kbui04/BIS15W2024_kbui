...
```{r}
sharks_dups2 <- filter(sharks_dups, injury == "fatal")
```
...
long3 <- c(-123.8, -117.3)
lat3 <- c(32.85, 39.58)
bbox3 <- make_bbox(long3, lat3, f = 0.05)
...
```{r}
ggmap(map3) + geom_point(data = sharks_dups2, aes(longitude, latitude), size = .4) + labs(title = "Fatal Shark Attacks", x = "Longitude", y ="Latitude")
```
