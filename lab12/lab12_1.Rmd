...
```{r}
homerange %>%
  ggplot(aes(x = log10.mass, y =class)) + geom_boxplot() + coord_flip()
```

2. Now, add a layer to this plot that fills color for each box by taxonomic class
```{r}
homerange %>%
  ggplot(aes(x = log10.mass, fill = class)) + geom_boxplot() + coord_flip()
```

3. Use `group` to show the range of `log10.mass` by taxonomic class.
```{r}
homerange %>%
  ggplot(aes(x = log10.mass, y = class, group_by(taxon))) + geom_boxplot()
```

4. Use `fill` to show the range of `log10.mass` by taxonomic class with color codes.
```{r}
homerange %>%
  ggplot(aes(x = log10.mass, y = class, fill = taxon)) + geom_boxplot() + coord_flip()
```
...
deserts %>%
  ...
  group_by(year, species_id) %>%
  summarize(n = n(), .groups = 'keep') %>%
  ggplot(aes(x = year, y =n, group = species_id, color = species_id)) + geom_line() + geom_point(shape = 2) + #you can experiment with shapes
...
```{r}
homerange %>%
  ggplot(aes(x = log10.hra)) + geom_histogram(bins = 40, fill = "yellow", color = "black", alpha = .4) + labs(title = "Distribution of Homerange")
```

2. Now plot the same variable using `geom_density()`.
```{r}
homerange %>%
  ggplot(aes(x = log10.hra)) + geom_density (color = "pink") + labs(title = "Distribution of Homerange")
```

3. Combine them both!
```{r}
homerange %>%
  ggplot(aes(x = log10.hra)) + geom_histogram(fill = "yellow", color = "black", alpha = .4, aes(y = after_stat(density))) + geom_density(color = "orange") + labs(title = "Distribution of Homerange")
```
...
homerange %>%
  ...
  ggplot(aes(x=mass_category, fill = trophic.guild)) + geom_bar(position = "dodge")
```
...
homerange %>%
  mutate(range_category = case_when(log10.hra < 3.65 ~ "very small", log10.hra >= 3.65 & log10.hra < 4.59 ~ "small", log10.hra >= 4.59 & log10.hra < 6.02 ~ "medium", log10.hra > 6.02 ~ "large")) %>%
  ggplot(aes(x = range_category, fill = class)) + geom_bar(position = "dodge", color = "black")
```
