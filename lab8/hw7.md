...
```r
amniota <- replace_with_na_all(data = amniota, condition = ~.x == -999)
```
...
Makes sense for mammals because most do not lay eggs, but all three most likely due to missing data

```r
amniota %>%
  group_by(class) %>%
  summarize(missing_egg_mass = sum(is.na(egg_mass_g)))
```

```
## # A tibble: 3 × 2
##   class    missing_egg_mass
##   <chr>               <int>
## 1 Aves                 4914
## 2 Mammalia             4953
## 3 Reptilia             6040
...
Could be that the authors could not identify the species behavior without enough data
...
```r
amniota <- read.csv("data/amniota.csv", na.strings = "-999")
```
