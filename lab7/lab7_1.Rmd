...
msleep24 <- select(msleep, name, vore) %>%
  mutate(sleep_total_24 = msleep$sleep_total/24) %>%
...
msleep %>%
  ...
  summarize(mean_bodywt_p = mean(bodywt), min_bodywt_p = min(bodywt), max_bodywt_p = max(bodywt), total = n())
```
...
```{r}
msleep %>%
  summarize(min_st = min(sleep_total), mean_st = mean(sleep_total), max_st = max(sleep_total), total = n())
```
